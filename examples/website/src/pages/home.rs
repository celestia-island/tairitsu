...
fn glow_btn(href: &str, class: &str, text: &str, arrow: Option<&str>) -> VNode {
    ...
    if let Some(arrow_text) = arrow {
        btn = btn.child(VNode::Element(Box::new(
            VElement::new("span")
                .class("btn-arrow")
    ...
    glow_wrapper(
        ...
        "soft",
        "rgba(20,110,116,0.5)",
        VNode::Element(Box::new(btn)),
    )
}

fn glow_card(title: &str, body: &str) -> VNode {
    let card = VNode::Element(Box::new(
        el("div")
            .class("card")
...
pub fn render() -> VNode {
    ...
    let actions = VNode::Element(Box::new(
        el("div")
            .class("page-hero__actions")
            .children(vec![btn1, btn2]),
    ));

    let hero_inner = VNode::Element(Box::new(
        el("div")
            .class("page-hero__inner")
            .children(vec![logo, title, subtitle, tagline, actions]),
    ));

    let hero_section = VNode::Element(Box::new(el("section").class("page-hero").child(hero_inner)));

    let section_title = VNode::Element(Box::new(
        el("h2")
    ...
    let card_grid = VNode::Element(Box::new(
        el("div")
            .class("card-grid")
            .children(vec![card1, card2, card3]),
    ));

    let section = VNode::Element(Box::new(
        el("section")
            .class("page-section")
