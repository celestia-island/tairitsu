...
/// Crate-specific integration points the shared implementation calls back
/// into. Each consuming crate binds its own logging front end and its own
/// Chromium executable resolver; `run_debug_server` moves it through the
/// engine (it is [`Copy`], so spawned tasks can capture it freely).
#[derive(Clone, Copy)]
pub struct DebugHost {
    /// Success/info completion line (packager `log_ok!` / browser
    /// `tracing::info!`).
    pub log_ok: fn(std::fmt::Arguments<'_>),
    /// Neutral progress line (packager `log_info!` / `tracing::info!`).
    pub log_info: fn(std::fmt::Arguments<'_>),
    /// Recoverable warning line (packager `log_warn!` / `tracing::warn!`).
    pub log_warn: fn(std::fmt::Arguments<'_>),
    /// Failure line (packager `log_fail!` / `tracing::error!`).
    pub log_fail: fn(std::fmt::Arguments<'_>),
    /// Resolve the Chromium/Chrome executable path. The returned error is the
    /// raw resolver error; the shared code wraps it with context.
    pub resolve_executable: fn() -> Result<String, String>,
}
...
mod engine {
    ...
    // `host` pushed this past clippy's 7-arg threshold; threading a struct
    // through would churn every call site for no clarity gain.
    ...
    pub(super) async fn spawn_browser(
        ...
        let exe = resolve_executable_blocking(host).await?;
        ...
        (host.log_ok)(format_args!("Debug browser connected (chromium raw-CDP)"));
        ...
        tokio::spawn(async move {
            ...
            *conn_reader.write().await = false;
            ...
            (host.log_warn)(format_args!("Chrome WebSocket disconnected — browser lost"));
        });
        ...
        if page_ok.is_err() {
            (host.log_warn)(format_args!(
                "Page.enable timed out — Chrome may be unstable in this environment"
            ));
        }
        let _ =
        ...
        match ping {
            Ok(Ok(val)) if val.as_u64() == Some(2) => {
                (host.log_ok)(format_args!("Chrome CDP verified responsive"));
            }
            _ => {
                (host.log_warn)(format_args!(
                    "Chrome CDP not responsive — browser operations may fail"
                ));
    ...
    fn resolve_executable(host: DebugHost) -> Result<String, String> {
    ...
    async fn resolve_executable_blocking(host: DebugHost) -> Result<String, String> {
        match tokio::task::spawn_blocking(move || resolve_executable(host)).await {
            Ok(res) => res,
...
/// full app config (the packager's `Config`) so the debug server can run
...
pub async fn run_debug_server(
    ...
    let (browser, browser_engine) = {
        (host.log_info)(format_args!(
            "Debug browser engine: chromium (headless CDP)"
        ));
        ...
        .await
        {
            ...
            Ok(Err(e)) => {
                (host.log_fail)(format_args!("[debug-browser] Failed: {e}"));
                (None, "none".to_string())
            }
            Err(_) => {
                (host.log_fail)(format_args!("[debug-browser] Timed out after 30s"));
                (None, "none".to_string())
            }
        }
    };
    ...
    (host.log_ok)(format_args!(
        "Debug API v{} listening on http://localhost:{}",
        DEBUG_API_VERSION, debug_port
    ));
    (host.log_info)(format_args!(
