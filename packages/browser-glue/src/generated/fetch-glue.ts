 ...
 * `set()` operation.
 */
export function _set(self: bigint, name: string, value: bigint): void {
  const obj = getHeaders(self);
  obj._set(name, value);
}
 ...
 * `get-body()` operation.
 */
export function getBody(self: bigint): number {
 ...
 */
export function formData(self: number): bigint {
  const requestId = _nextAsyncHandle++;
  const obj = getBody(self);
 ...
 */
export function json(self: bigint): boolean {
  const requestId = _nextAsyncHandle++;
  const obj = getBody(self);
 ...
 * Returns undefined if still pending, or the result if complete.
 */
export function pollJson(requestId: bigint): { ok: true; value: boolean } | { ok: false; error: string } | undefined {
  const entry = _asyncHandles.get(requestId);
  if (!entry) {
    return { ok: false, error: `Unknown request ID ${requestId}` };
  }
...
/** Type alias */
export type RequestHandle = (bigint)[] | undefined;
 ...
 * `get-mode()` operation.
 */
export function getMode(self: boolean): bigint {
  const obj = getRequest(self);
  return obj.mode;
}
 ...
 * `get-cache()` operation.
 */
export function getCache(self: bigint): string {
  const obj = getRequest(self);
  return obj.cache;
}
 ...
 */
export function getRedirect(self: bigint): string {
  const obj = getRequest(self);
  return obj.redirect;
}
 ...
 */
export function getIntegrity(self: string | undefined): bigint {
  const obj = getRequest(self);
 ...
 */
export function getIsReloadNavigation(self: bigint | undefined): boolean {
  const obj = getRequest(self);
 ...
 * `redirect()` operation.
 */
export function redirect(url: number, status: bigint | undefined): bigint {
 ...
 * Async operation: returns request ID, poll with `pollJson()`
 */
export function json(data: string, init: string): string {
  const requestId = _nextAsyncHandle++;
  const obj = getResponse(self);
