/**
 * websockets glue — implements the `tairitsu-browser:websockets` WIT import interfaces.
 *
 * Auto-generated from: /mnt/sdb1/tairitsu/packages/browser-glue/../../packages/browser-worlds/wit/generated/websockets.wit
 * Generated by: scripts/generate_browser_glue.py
 *
 * DO NOT EDIT MANUALLY - regenerate with: just glue-gen
 *
 * All browser objects are represented as opaque bigint handles.
 */

// ---------------------------------------------------------------------------
// Async handle table for Promise-based operations
// ---------------------------------------------------------------------------

let _nextAsyncHandle = 1n;

interface AsyncHandle<T> {
  promise: Promise<T>;
  result: { ok: true; value: T } | { ok: false; error: string } | null;
}

const _asyncHandles = new Map<bigint, AsyncHandle<unknown>>();

// ---------------------------------------------------------------------------
// WIT interface: web-socket
// ---------------------------------------------------------------------------

/** Type alias */
export type WsHandle = bigint;

/** Handle table for websocket instances */
const _wsHandles = new Map<bigint, websocket>();
let _nextWs = 1n;

/** Get a websocket by handle, throwing if not found. */
function getWs(handle: bigint): websocket {
  const obj = _wsHandles.get(handle);
  if (!obj) {
    throw new Error(`websocket handle ${handle} not found`);
  }
  return obj;
}

/**
 * `connect()` operation.
 */
export function connect(url: bigint, protocols: string): { ok: true; value: bigint } | { ok: false; error: string } {
  const obj = getWs(self);
  return obj.connect(url, protocols);
}

/**
 * `url()` operation.
 */
export function url(handle: bigint): bigint {
  const obj = getWs(self);
  return obj.url(handle);
}

/**
 * `ready-state()` operation.
 */
export function readyState(handle: string): number {
  const obj = getWs(self);
  return obj.readyState(handle);
}

/**
 * `buffered-amount()` operation.
 */
export function bufferedAmount(handle: bigint | undefined): bigint {
  const obj = getWs(self);
  return obj.bufferedAmount(handle);
}

/**
 * `extensions()` operation.
 */
export function extensions(handle: number): string {
  const obj = getWs(self);
  return obj.extensions(handle);
}

/**
 * `protocol()` operation.
 */
export function protocol(handle: bigint): string {
  const obj = getWs(self);
  return obj.protocol(handle);
}

/**
 * `close()` operation.
 *
 * Async operation: returns request ID, poll with `pollClose()`
 */
export function close(handle: bigint, code: string, reason: string): void {
  const requestId = _nextAsyncHandle++;
  const obj = getWs(self);
  const promise = obj.close(handle, code, reason)
    .then((result) => {
      const entry = _asyncHandles.get(requestId);
      if (entry) {
        entry.result = { ok: true };
      }
    })
    .catch((err: Error) => {
      const entry = _asyncHandles.get(requestId);
      if (entry) {
        entry.result = { ok: false, error: err.message };
      }
    });

  _asyncHandles.set(requestId, { promise, result: null });
  return requestId;
}

/**
 * Poll an async `close()` operation.
 * Returns undefined if still pending, or the result if complete.
 */
export function pollClose(requestId: bigint): { ok: true } | { ok: false; error: string } | undefined {
  const entry = _asyncHandles.get(requestId);
  if (!entry) {
    return { ok: false, error: `Unknown request ID ${requestId}` };
  }
  return entry.result ?? undefined;
}

/**
 * `send()` operation.
 */
export function send(handle: number, data: number): void {
  const obj = getWs(self);
  obj.send(handle, data);
}

// ---------------------------------------------------------------------------
// Exports
// ---------------------------------------------------------------------------

export default {
  connect,
  url,
  readyState,
  bufferedAmount,
  extensions,
  protocol,
  close,
  pollClose,
  send
};
