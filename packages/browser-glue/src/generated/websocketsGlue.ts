 ...
 * `connect()` operation.
 */
export function connect(url: bigint | undefined, protocols: bigint): { ok: true; value: bigint } | { ok: false; error: string } {
 ...
 * `close()` operation.
 */
export function close(handle: bigint, code: number, reason: boolean): void {
  const obj = lookupWs(handle);
