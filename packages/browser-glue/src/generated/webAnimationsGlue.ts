/**
 * web-animations glue — implements the `tairitsu-browser:web-animations` WIT import interfaces.
 *
 * Auto-generated from: /mnt/sdb1/tairitsu/packages/browser-glue/../../packages/browser-worlds/wit/generated/web-animations.wit
 * Generated by: scripts/generate_browser_glue.py
 *
 * DO NOT EDIT MANUALLY - regenerate with: just glue-gen
 *
 * All browser objects are represented as opaque bigint handles.
 */

// ---------------------------------------------------------------------------
// Async handle table for Promise-based operations
// ---------------------------------------------------------------------------

let _nextAsyncHandle = 1n;

interface AsyncHandle<T> {
  promise: Promise<T>;
  result: { ok: true; value: T } | { ok: false; error: string } | null;
}

const _asyncHandles = new Map<bigint, AsyncHandle<unknown>>();

// ---------------------------------------------------------------------------
// WIT interface: animation
// ---------------------------------------------------------------------------

/** Type alias */
export type AnimationHandle = bigint;

/** Handle table for animation instances */
const _animationHandles = new Map<bigint, animation>();
let _nextAnimation = 1n;

/** Get a animation by handle, throwing if not found. */
function getAnimation(handle: bigint): animation {
  const obj = _animationHandles.get(handle);
  if (!obj) {
    throw new Error(`animation handle ${handle} not found`);
  }
  return obj;
}

/**
 * `get-id()` operation.
 */
export function getId(handle: string): bigint {
  const obj = getAnimation(self);
  return obj.id;
}

/**
 * `set-id()` operation.
 */
export function setId(handle: (bigint)[], value: boolean): void {
  const obj = getAnimation(self);
  obj.id = value;
}

/**
 * `get-playback-rate()` operation.
 */
export function getPlaybackRate(handle: string): number {
  const obj = getAnimation(self);
  return obj.playbackRate;
}

/**
 * `set-playback-rate()` operation.
 */
export function setPlaybackRate(handle: string | undefined, value: string | undefined): void {
  const obj = getAnimation(self);
  obj.playbackRate = value;
}

/**
 * `pending()` operation.
 */
export function pending(handle: bigint): number {
  const obj = getAnimation(self);
  return obj.pending(handle);
}

/**
 * `cancel()` operation.
 *
 * Async operation: returns request ID, poll with `pollCancel()`
 */
export function cancel(handle: string): void {
  const requestId = _nextAsyncHandle++;
  const obj = getAnimation(self);
  const promise = obj.cancel(handle)
    .then((result) => {
      const entry = _asyncHandles.get(requestId);
      if (entry) {
        entry.result = { ok: true };
      }
    })
    .catch((err: Error) => {
      const entry = _asyncHandles.get(requestId);
      if (entry) {
        entry.result = { ok: false, error: err.message };
      }
    });

  _asyncHandles.set(requestId, { promise, result: null });
  return requestId;
}

/**
 * Poll an async `cancel()` operation.
 * Returns undefined if still pending, or the result if complete.
 */
export function pollCancel(requestId: bigint): { ok: true } | { ok: false; error: string } | undefined {
  const entry = _asyncHandles.get(requestId);
  if (!entry) {
    return { ok: false, error: `Unknown request ID ${requestId}` };
  }
  return entry.result ?? undefined;
}

/**
 * `finish()` operation.
 */
export function finish(handle: string | undefined): void {
  const obj = getAnimation(self);
  obj.finish(handle);
}

/**
 * `play()` operation.
 */
export function play(handle: number): void {
  const obj = getAnimation(self);
  obj.play(handle);
}

/**
 * `pause()` operation.
 */
export function pause(handle: string): void {
  const obj = getAnimation(self);
  obj.pause(handle);
}

/**
 * `update-playback-rate()` operation.
 */
export function updatePlaybackRate(handle: bigint, playbackRate: number): void {
  const obj = getAnimation(self);
  obj.updatePlaybackRate(handle, playbackRate);
}

/**
 * `reverse()` operation.
 */
export function reverse(handle: number): void {
  const obj = getAnimation(self);
  obj.reverse(handle);
}

/**
 * `persist()` operation.
 *
 * Async operation: returns request ID, poll with `pollPersist()`
 */
export function persist(handle: bigint): void {
  const requestId = _nextAsyncHandle++;
  const obj = getAnimation(self);
  const promise = obj.persist(handle)
    .then((result) => {
      const entry = _asyncHandles.get(requestId);
      if (entry) {
        entry.result = { ok: true };
      }
    })
    .catch((err: Error) => {
      const entry = _asyncHandles.get(requestId);
      if (entry) {
        entry.result = { ok: false, error: err.message };
      }
    });

  _asyncHandles.set(requestId, { promise, result: null });
  return requestId;
}

/**
 * Poll an async `persist()` operation.
 * Returns undefined if still pending, or the result if complete.
 */
export function pollPersist(requestId: bigint): { ok: true } | { ok: false; error: string } | undefined {
  const entry = _asyncHandles.get(requestId);
  if (!entry) {
    return { ok: false, error: `Unknown request ID ${requestId}` };
  }
  return entry.result ?? undefined;
}

/**
 * `commit-styles()` operation.
 */
export function commitStyles(handle: string | undefined): void {
  const obj = getAnimation(self);
  obj.commitStyles(handle);
}

// ---------------------------------------------------------------------------
// Exports
// ---------------------------------------------------------------------------

export default {
  getId,
  setId,
  getPlaybackRate,
  setPlaybackRate,
  pending,
  cancel,
  pollCancel,
  finish,
  play,
  pause,
  updatePlaybackRate,
  reverse,
  persist,
  pollPersist,
  commitStyles
};
