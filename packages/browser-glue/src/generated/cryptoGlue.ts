 ...
 */
export function deriveBits(self: bigint, algorithm: (string)[], baseKey: bigint, length: number | undefined): bigint {
  const requestId = _nextAsyncHandle++;
  const obj = lookupSubtleCrypto(self);
 ...
 */
export function importKey(self: bigint, format: string, keyData: Uint8Array, algorithm: bigint, extractable: bigint, keyUsages: number): bigint {
  const requestId = _nextAsyncHandle++;
  const obj = lookupSubtleCrypto(self);
 ...
 */
export function unwrapKey(self: bigint, format: bigint, wrappedKey: bigint, unwrappingKey: bigint, unwrapAlgorithm: string, unwrappedKeyAlgorithm: bigint, extractable: boolean, keyUsages: (bigint)[]): bigint {
  const requestId = _nextAsyncHandle++;
  const obj = lookupSubtleCrypto(self);
