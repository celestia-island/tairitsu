 ...
 */
export function verify(self: bigint, algorithm: bigint, key: bigint, signature: boolean, data: Uint8Array): bigint {
  const requestId = _nextAsyncHandle++;
  const obj = lookupSubtleCrypto(self);
 ...
 */
export function digest(self: bigint, algorithm: number, data: Uint8Array): bigint {
  const requestId = _nextAsyncHandle++;
  const obj = lookupSubtleCrypto(self);
