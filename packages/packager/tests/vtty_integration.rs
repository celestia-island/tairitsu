//! Integration tests for VTty — PTY lifecycle (spawn → read → screenshot → kill)
//!
//! Run with: cargo test -p tairitsu-packager --features vtty --test vtty_integration
...
fn test_pty_spawn_and_kill() {
    let (mut pty, pid) = tairitsu_packager::vtty::pty_win::ConPty::spawn(
        TEST_CMD, 80, 24, None,
...
fn test_pty_write_and_read() {
    let (mut pty, _) = tairitsu_packager::vtty::pty_win::ConPty::spawn(
        TEST_CMD, 80, 24, None,
    ).expect("ConPty spawn failed");
...
fn test_session_lifecycle() {
    ...
    let info = mgr.launch(TEST_CMD, 80, 24, "", None, "integration-test")
    ...
    mgr.kill(&info.id).ok(); // ensure removal from map
    assert!(mgr.list().is_empty());
...
fn test_session_screenshot_after_echo() {
    ...
    let info = mgr.launch(TEST_CMD, 80, 24, "", None, "screenshot-test")
        .expect("launch failed");
...
fn test_send_keys_arrow_keys() {
    ...
    assert_eq!(bytes.len(), 18); // 6 sequences x 3 bytes each
}
...
fn test_multiple_sessions() {
    ...
    let a = mgr.launch(TEST_CMD, 80, 24, "", None, "sess-a").expect("launch a failed");
    let b = mgr.launch(TEST_CMD, 80, 24, "", None, "sess-b").expect("launch b failed");
    assert_ne!(a.id, b.id);
