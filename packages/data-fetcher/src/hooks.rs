...
pub fn use_fetch_with_fetcher<T, F, Fr>(url: &str, parser: F, fetcher: Fr) -> Rc<RefCell<Resource<T>>>
where
    T: Clone + Send + 'static,
    F: Fn(&[u8]) -> Result<T, String> + Clone + Send + 'static,
...
where
    ...
    Fr: Fetcher + Clone + Send + Sync + 'static,
{
    use_fetch_with_fetcher(url, |data| {
        serde_json::from_slice(data).map_err(|e| e.to_string())
    }, fetcher)
}
...
mod tests {
    ...
    fn test_use_fetch_creates_resource() {
        ...
        let resource: Rc<RefCell<Resource<String>>> =
            use_fetch("http://example.com", |data| {
