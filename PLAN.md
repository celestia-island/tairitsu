...
| 项目 | 测试总数 | 通过率 | 问题 |
|------|----------|--------|------|
| **Tairitsu** | 940+ `#[test]` 标注 | **1 失败** (browser-wit-resolver) | diff.rs 20 个测试；registry 11 个测试；container 6 个测试；wit.rs 只有一个空体 smoke test |
| **Hikari** | 836+ 通过 (25 binaries) | **0 失败** | basic_components_tests 升级为结构化断言（find_elements_by_tag + 属性验证）；无无障碍测试；animation 23 个 doctest 全被 ignore |
...
- [x] **[Both] CI 添加 `cargo audit` + `cargo deny`** — Hikari 已添加 `.github/workflows/security.yml`；Tairitsu 侧待添加
...
- [x] **[Hikari] 统一 CSS 变量命名** — 18 个非 `--hi-` 前缀变量已全部迁移到 `--hi-`
...
- [ ] **[Hikari] 清理悬空依赖**（`once_cell`、`chrono`）— `gloo`/`gloo-net` 已移除
- [ ] **[Tairitsu] Upstream `load_toml_flat`** — 将 hikari `examples/website/src/i18n_init.rs:59-92` 中的 TOML→`HashMap<String,String>` 扁平化工具提取到 `tairitsu-packager/src/i18n/` 或 `tairitsu-web` 作为公共 API。当前 hikari website 内联了一份递归展平器（将嵌套 TOML table 展开为 `key.subkey = value` 形式的 dot-separated map），应抽为 tairitsu-web 的 `i18n` 模块导出函数。上游完成后，从 hikari website 中删除内联副本。
