...
| C2 | Tairitsu | `vdom/src/diff.rs:109-140` | ~~**key-based reconciliation 未实现**~~ **已修复** — 实现了 LIS 启发式 key 匹配算法，包含 MoveChild/ReorderChildren 补丁类型，支持有 key/无 key 混合子节点 | 列表排序/重排性能极差，组件状态在重排后丢失 |
...
- [ ] **[Hikari] 补齐键盘无障碍** — 为 Switch、Tabs、Menu、Select 添加 Enter/Space/Arrow 键盘事件处理器（Tabs/Switch 已部分实现，Menu/Select 待补齐）
